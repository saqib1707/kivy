...
class TextInput(FocusBehavior, Widget):
    ...
    def get_cursor_from_xy(self, x, y):
        ...
        for i in range(0, len(z[cy])):
            if (_get_text_width(z[cy][:i], _tab_width, _label_cached) +
                  _get_text_width(z[cy][i], _tab_width, _label_cached) * 0.6 +
    ...
    def _get_text(self, encode=False):
        ...
        text = u''.join([(u'\n' if (lf[i] & FL_IS_LINEBREAK) else u'') + z[i]
